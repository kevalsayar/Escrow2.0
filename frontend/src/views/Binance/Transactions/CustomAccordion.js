...
function CustomAccordion(props) {
  ...
  //   const weiAmount = weiFunctions(`${amount}`, WEI.FROM_WEI);
  //   return weiAmount;
  // }

  useEffect(() => {
    setamount(weiFunctions(`${amount}`, WEI.FROM_WEI));
    // convertToWei(amount).then((value) => {
    //   setamount(value);
    // });
  }, []);
  ...
  const handleWithdrawClick = async () => {
    ...
    if (await switchChain(NETWORK_CHAINS.BINANCE_TEST_NETWORK)) {
      ...
      sendSmartContract(
        CONTRACT_FUNCTIONS.BSC.ESCROW.WITHDRAW,
        [],
  ...
  const handleReleaseClick = async () => {
    ...
    if (await switchChain(NETWORK_CHAINS.BINANCE_TEST_NETWORK)) {
      ...
      sendSmartContract(
        CONTRACT_FUNCTIONS.BSC.ESCROW.RELEASE,
        [],
        [{ from: (await fetchAccounts())[0] }],
        escrowProxyAbi,
        obj?.escrow_wallet
